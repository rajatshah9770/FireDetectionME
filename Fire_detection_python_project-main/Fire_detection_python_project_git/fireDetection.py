...
while(True):
    ...
    for (x,y,w,h) in fire:
        ...
        if runOnce == False:
            ...
            threading.Thread(target=send_mail_function).start() # To call SMS thread
            '''
            ############### Remove comment to use sms service ##################
            client = Client(account_sid, auth_token) # To send SMS once
            message = client.messages.create(
            to="+918624049464", 
            from_="+16075644952",
            body="Alert...! Fire is detetced")
            print(message.sid)
            print("SMS is sent")
            ############### Remove comment till here to use sms service ##################
            '''
            runOnce = True
        if runOnce == True:
            print("Mail and SMS is already sent once")
            runOnce = True
